...
bool KisApplication::registerResources()
{
    ...
    if (!KisResourceCacheDb::initialize(databaseLocation)) {
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Ultra: Fatal error"), i18n("%1\n\nUltra will quit now.", KisResourceCacheDb::lastError()));
    }
    ...
    if (r != KisResourceLocator::LocatorError::Ok && qApp->inherits("KisApplication")) {
        QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Ultra: Fatal error"), KisResourceLocator::instance()->errorMessages().join('\n') + i18n("\n\nKrita will quit now."));
        return false;
    }
    return true;
}
...
bool KisApplication::start(const KisApplicationArguments &args)
{
    ...
    if (isWow64() && !cfg.readEntry("WarnedAbout32Bits", false)) {
        QMessageBox::information(qApp->activeWindow(),
                                 i18nc("@title:window", "Ultra: Warning"),
    ...
    themeManager.setCurrentTheme(group.readEntry("Theme", "Ultra dark"));
#endif
    ...
    if (!args.fileLayer().isEmpty()){
        if (d->mainWindow->viewManager()->image()){
            ...
            QMessageBox::warning(qApp->activeWindow(), i18nc("@title:window", "Ultra:Warning"),
...
"If you instead want to add the file layer to a document in an already running instance of Ultra, check the \"Allow only one instance of Ultra\" checkbox in the settings (Settings -> General -> Window)."));
        }
        else {
            QMessageBox::warning(qApp->activeWindow(), i18nc("@title:window", "Ultra: Warning"),
                                i18n("Cannot add the file layer: no document is open.\n"
...
void KisApplication::executeRemoteArguments(QByteArray message, KisMainWindow *mainWindow)
{
    ...
    if (!args.fileLayer().isEmpty()){
        if (argsCount > 0  && !documentCreated){
            ...
            QMessageBox::warning(mainWindow, i18nc("@title:window", "Ultra:Warning"),
...
void KisApplication::checkAutosaveFiles()
{
    ...
    QStringList filters = QStringList() << QString(".ultra-*-*-autosave.ult");

    // all autosave files for our application
    QStringList autosaveFiles = dir.entryList(filters, QDir::Files | QDir::Hidden);

    // Visible autosave files
    filters = QStringList() << QString("ultra-*-*-autosave.ult");
...
bool KisApplication::createNewDocFromTemplate(const QString &fileName, KisMainWindow *mainWindow)
{
    ...
    if (QFile::exists(fileName)) {
        ...
        if (paths.isEmpty()) {
            QMessageBox::critical(qApp->activeWindow(), i18nc("@title:window", "Ultra"),
        ...
        if (kritarcFile.open(QFile::ReadWrite)) {
            ...
            QMessageBox::information(qApp->activeWindow(),
                                 ...
                                 i18n("Krita configurations reset!\n\n"
                                      ...
                                      "Restart Ultra for changes to take effect.",
                                      backupKritarcPath),
